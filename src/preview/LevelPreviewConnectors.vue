...
const connectors = computed(() =>
    [...cullEntities('connector', keys.value.min, keys.value.max)]
        // .filter(({ head, tail }) => 
        //     tail.beat >= beats(tail.groupId).value.min ||
        //     head.beat < beats(head.groupId).value.max)
        ...
        .map(({ head, tail }) => {
            ...
            const targetTime = {
                head: timeToScaledTime(timeScales.value.get(head.groupId)!, time.head),
                tail: timeToScaledTime(timeScales.value.get(tail.groupId)!, time.tail)
            }
            ...
            const scaled = {
                head: timeToScaledTime(timeScales.value.get(head.groupId)!, Math.min(view.cursorTime, time.head)),
                tail: timeToScaledTime(timeScales.value.get(tail.groupId)!, Math.min(view.cursorTime, time.tail))
            }
            const s = {
                head: clamp(unlerp(targetTime.head - (nsh ? 5 / nsh : noteDuration.value), targetTime.head, scaled.head), 0, 2),
                tail: clamp(unlerp(targetTime.tail - (nst ? 5 / nst : noteDuration.value), targetTime.tail, scaled.tail), 0, 2),
            }
            //console.log(s)
            const size = {
            ...
            if (head.lane === tail.lane) {
                ...
                const scaled2 = {
                    head: timeToScaledTime(timeScales.value.get(head.groupId)!, view.cursorTime),
                    tail: timeToScaledTime(timeScales.value.get(tail.groupId)!, view.cursorTime)
                }

                const visibleTime = {
                    min: view.cursorTime > time.head ? scaled2.head : targetTime.head,
                    max: Math.min(scaled2.tail + noteDuration.value, targetTime.tail)
                }

                const segs: { points: string, fill: string, opacity: string }[] = []
                const div = Math.ceil(Math.abs(s.head - s.tail) * 20)
                //console.log("div", div, t)
                console.log(visibleTime, scaled2, div)
                for (let i = 0; i < div; i++) {
                    const scaledTime = {
                    ...
                    const s = {
                        min: unlerp(scaledTime.min - (nsh ? 5 / nsh : noteDuration.value), scaledTime.min, scaled2.head),
                        max: unlerp(scaledTime.max - (nst ? 5 / nst : noteDuration.value), scaledTime.max, scaled2.tail),
                    }
